...
`addTable()` accepts a non-empty dense rectangular matrix of single-paragraph strings or strict `{ text, options? }` objects whose supported cell options are align, border, fill, fit, hyperlink, margin, text direction, and vertical alignment. Table defaults for align, border, fill, margin, text direction, and vertical alignment are materialized only into uncovered physical cells; cell values win and no creation metadata is retained. There is no table-level hyperlink default. Inputs are descriptor-safe, getter-free, detached, and serialized in stable `tcPr` order: margins, optional anchor/direction, L/R/T/B borders, then fill; a cell hyperlink is attached to its one direct text run. Geometry uses EMU, with strict scalar or exact-length column/row vectors and synchronized transform dimensions. Existing tables expose cell-level direct snapshots/editors plus readonly direct hyperlink snapshots and table-level uniform consensus/all-physical-cell bulk editing through `verticalAlignment`, `textDirection`, `horizontalAlignment`, `margins`, `borders`, and `fill`; mixed detail remains under `rows[].cells[]`, and explicit direction `horz`, direct no-fill, zero-width line, omitted border style, explicit solid, and explicit-zero alpha stay distinct from `undefined` clear. Cell options outside the listed set—including merge and rich or multi-paragraph text—plus hyperlink editing/clearing, table-level fit creation/defaults, diagonal/advanced borders, advanced fills, table creation styles, auto-page, repeated headers, and content measurement remain unsupported and are rejected rather than ignored. The detailed sections below define exact creation precedence, numeric ranges, OOXML mappings, no-op behavior, and PptxGenJS boundaries for each property.

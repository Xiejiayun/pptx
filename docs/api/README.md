...
Native rejects deprecated `addHeaderToEach` / `newSlideStartY`, coercible controls, malformed margins, and unsupported fragmentation boundaries, and does not copy PptxGenJS 4.0.1 caller mutation, weight clamping, or following-slide reuse. Current overall PptxGenJS parity is approximately 99.7%. The only remaining capability item is `tableToSlides`, followed by the final peer/client audit; 100% or full parity is not yet claimed.

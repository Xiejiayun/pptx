...
Table-cell creation names the option `AddTableCellOptions.textDirection` and accepts only `horz`, `vert`, `vert270`, or `wordArtVert`. It owns the selected physical cell's direct `tcPr@vert`, never `bodyPr@vert`; omitted, runtime-undefined, and explicit `horz` create no direct attribute, while the three non-horizontal values write their exact token and are exposed immediately by the live `TableCell.textDirection` snapshot. Inputs follow the same descriptor-safe, getter-free, detached normalization as other cell options, and invalid or text-box-only tokens are rejected before mutation. The direct state survives supported cell edits, slide duplication, rollback, write, and reopen. `setCellTextDirection()` addresses physical zero-based row/cell positions and retains lossless editor semantics: explicit `horz` writes `vert="horz"`, while `undefined` clears the direct attribute. The getter requires one direct `tcPr`, one unqualified `vert`, and one exact public token; it does not resolve table defaults or inheritance. PptxGenJS 4.0.1 produces the same supported cell-level final state and likewise collapses creation omission and explicit `horz`; its runtime invalid-token passthrough remains unsupported by the strict native API. This four-value cell API is separate from the seven-value text-box API, and a table-level direction default remains unsupported.

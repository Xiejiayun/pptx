...
Overall parity remains approximately 97%. Table-level direct border consensus read/bulk edit is next, followed by the remaining advanced text/table work, `tableToSlides`, and the final peer/client audit.
...
Shape kinds include `text`, `shape`, `image`, `table`, `chart`, `graphic-frame`, and `group`. Images expose embedded part URIs and replacement; tables support basic native creation plus rows/cells, cell text, borders, fill, margins, cell horizontal/vertical alignment, table-level vertical-alignment consensus/bulk editing, text-direction, and text-fit editing; charts expose cached series and lossless chart XML editing.
...
`addTable()` accepts a non-empty dense rectangular matrix of single-paragraph strings or strict `{ text, options? }` objects whose supported cell options are align, border, fill, fit, margin, text direction, and vertical alignment. Table defaults for align, border, fill, margin, text direction, and vertical alignment are materialized only into uncovered physical cells; cell values win and no creation metadata is retained. Inputs are descriptor-safe, getter-free, detached, and serialized in stable `tcPr` order: margins, optional anchor/direction, L/R/T/B borders, then fill. Geometry uses EMU, with strict scalar or exact-length column/row vectors and synchronized transform dimensions. Existing tables expose cell-level direct snapshots/editors plus table-level uniform consensus and all-physical-cell bulk editing through `verticalAlignment`, `textDirection`, `horizontalAlignment`, `margins`, and `fill`; mixed detail remains under `rows[].cells[]`, and explicit direction `horz`, direct no-fill, and explicit-zero alpha stay distinct from `undefined` clear. Cell options outside the listed set—including merge, hyperlinks, and rich or multi-paragraph text—plus table-level fit creation/defaults, the table-level border getter/editor, diagonal/advanced borders, advanced fills, table creation styles, auto-page, repeated headers, and content measurement remain unsupported and are rejected rather than ignored. The detailed sections below define exact creation precedence, numeric ranges, OOXML mappings, no-op behavior, and PptxGenJS boundaries for each property.

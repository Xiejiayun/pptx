...
Full theme text cascade, percentage coordinates, advanced text/table/media/chart styles, and broad client certification remain pending. Advanced text now includes text-shape direct fill, simple line, arrows, simple shadow, outer hyperlink, and per-run rich-text hyperlink creation/read/edit.

`AddTextOptions.shape` text geometry is next, followed by `rectRadius` / `isTextBox` / `breakLine`, the remaining advanced-text items, advanced table/`tableToSlides`, output/runtime helpers, and the peer-range full-suite audit.

...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. Six-value `write({ outputType })` return semantics are completed below.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. Node readable output is completed below.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. Compression policy is completed below.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. The `SchemeColor` runtime helper is completed below.
...
Raster and SVG creation now share `name`, `altText`, `rounding`, `transparency`, and strict `ShapeShadow` options. Live `ImageModel.name`, `.altText`, `.rounding`, `.transparency`, and `.shadow` support safe direct editing, exact no-op, rollback, duplicate isolation, and write/reopen without changing image relationships or media bytes. Rounding maps to rectangular/ellipse geometry; transparency is a quantized `0..100` percentage; shadow supports legal outer/inner state. PptxGenJS `objectName` maps to native `name`, while native shadow deliberately preserves legal explicit zero and rotate-with-shape state.

Raster and SVG image hyperlinks support create, read, edit, duplicate, rollback, and reopen. SVG DOM editing, external SVG relationships, and broader advanced SVG operations remain native-extension boundaries. Picture-placeholder population itself is supported by the named master/layout API below. Strict embedded media creation is documented in the Media section below.
...
`ShapeLine` supports direct none or a solid sRGB/theme line with optional finite 0–100 transparency, optional 0–1584 point width, optional `solid | dash | dashDot | lgDash | lgDashDot | lgDashDotDot | sysDash | sysDot` dash, and optional `flat | round | square` cap. Omitted width/dash materialize as 1pt/solid; zero width remains direct zero. `ShapeModel.line` returns a detached direct-state snapshot; same-value assignment is an exact no-op, `{ kind: 'none' }` writes direct line no-fill, and `undefined` clears only owned width/fill/dash/cap while preserving the line container, arrowheads, joins, extensions, and unrelated attributes. Existing advanced line fills and custom dashes survive unrelated edits and can be explicitly replaced or cleared, but their creation remains outside this simple-line API.
...
`AddTextOptions.line?: ShapeLine` is normalized before package mutation. The union is exactly direct none or a solid line with required strict six-digit sRGB/supported scheme color plus optional finite `0..100` transparency, finite `0..1584` point width, one of eight preset dash tokens, and `flat | round | square` cap. Transparency is rounded to `0.001%`, width to one EMU, and omitted width/dash materialize as 1pt/solid. Omitted, runtime-`undefined`, and explicit none creation preserve canonical direct text-box no-fill; explicit zero transparency writes `a:alpha val="100000"` and zero width writes `a:ln@w="0"`. PptxGenJS-shaped aliases, missing color, invalid range/token, non-ordinary objects, unknown/inherited/accessor/symbol keys, and class instances reject before parts, relationships, XML, caches, shape order, or the mutation journal change.

Plain/rich text, `addPlaceholder()`, named placeholder population, layout/master wrapper methods, and declarative `defineSlideMaster()` text/placeholder objects share one renderer. Line stays after geometry and shape fill. The resulting `ShapeModel.line` immediately returns a detached normalized snapshot and keeps the existing read/whole-replace/clear semantics: same value is an exact no-op, none writes direct no-fill, and `undefined` removes owned width/fill/dash/cap while preserving the line container and unrelated state. Duplicate, move, outer rollback, six-format write/reopen, stable identity, sibling isolation, and placeholder-source isolation are covered.

PptxGenJS 4.0.1 emits empty `a:ln` for omitted/none/empty/missing-color text line, omits direct width/dash for their defaults, collapses width/transparency zero, honors nested deprecated `alpha`, and ignores nested `lineDash`. Native uses explicit reversible direct state and rejects permissive aliases/fallbacks. Supported sRGB/theme, non-zero transparency, positive width, all eight dash tokens, and flat/round/square caps reach equivalent final semantics. Gradient/pattern/picture/group line fills, custom dash, and compound/alignment/join editing stay outside this simple-line API. Text arrows, simple shadow, hyperlink, and preset geometry are supported below.
...
`CHART_TYPES` is the frozen public catalog of `area`, `bar`, `bar3D`, `bubble`, `bubble3D`, `doughnut`, `line`, `pie`, `radar`, and `scatter`. `SlideModel.addChart()` and `PptxDocument.addChart(slideIndex, ...)` expose the same overloads:
...
PptxGenJS 4.0.1 conformance covers all ten public chart types, bar+line primary/secondary combinations, data/formulas/caches/XLSX relationships, and representative title/legend/axis/label/data-table/series/type-specific options. The final actual-npm aggregate contains 21 slides and 20 native charts; Node, persistent Chrome, declarations, and the installed CLI pass, with 0 PowerPoint-2010 validation errors/warnings. The earlier 11-slide / 10-chart visual gallery has zero 180-DPI overflow and passed page-by-page review; it is retained as historical visual evidence, not as the final `bubble3D` / Chart Residual aggregate proof. LibreOffice retains chart types and cached data but removes workbooks on save; these reopen as cache-only warnings and can be canonicalized by semantic editing. Its `bar3D` renderer is title-only for both native and PptxGenJS controls. Built-in trendline/error-bar creation and modern inspection remain in the advanced-charts plugin.
...
Full theme text cascade, broader advanced text/table/media/chart semantic APIs, and broad client certification remain native-extension or independent-certification boundaries. Percentage coordinates are supported. Advanced text includes text-shape direct fill, simple line, arrows, simple shadow, outer hyperlink, per-run rich-text hyperlink, preset geometry, rounded-rectangle-radius, direct `isTextBox` creation/read/edit, and rich-text `breakLine` paragraph splitting.

This paragraph records the earlier master/layout checkpoint. The later advanced-text, advanced-table, `tableToSlides`, output/runtime, and full-suite workstreams are complete; PptxGenJS 4.0.1 public-surface parity is complete, while broad independent client/peer certification remains separate.
...
The API does not retain DOM/CSS objects, parse word-level HTML, or map alpha into table fill/borders. A hidden table without sufficient fixed-width constraints rejects. Native deliberately avoids PptxGenJS 4.0.1 selector interpolation, caller mutation, ignored `autoPage: false`, truthy coercion, fixed-width-as-minimum behavior, and silent invalid numbers.

The workstream is 10/10 complete. Focused/full/performance gates are 4 files / 463 tests in 57.92s, 91 passed / 1 skipped files with 1764 passed / 1 skipped tests in 102.00s, and 1/1 with a 1.145s core test. Two builds produce the same 63-file dist manifest (`280e78ad56ad3cb5891b80a729d2a899bfd42f3e77648517c1982df71a63bc3a`) and byte-identical 66-entry, 712,812-byte tarballs (`2fbd19e234d9e92591c0825acd6082bfda394ff871aa58d715c20d71f2e1d623`). Installed Node, NodeNext, browser, CLI, and Inspector checks pass; Chrome 150.0.7871.188 reports every `tableToSlides` state true and zero validation/console/page/network errors.

The Node evidence deck has 29 parts / 41 relationships and 5 slides / 4 generated pages (`923b84490d4e588d32d4a91cb55f2df37478d9b3e2b2ce5113285a2992b9f1e2`); the Chrome deck has 33 parts / 53 relationships and 7 slides / 6 generated pages (`c60b0665cee91dfe114f7aebb537f2a13b933e7e0577d23c81486ac2eef6495a`). They validate with zero errors and only 8/12 expected external-link warnings, render all 12 pages at 2400×1350 with zero overflow, and survive LibreOffice 26.8 save/reopen with zero errors. Local PowerPoint 16.112 automation returns `-9074` without an output deck, so no local PowerPoint round-trip pass is claimed. Public capability coverage is 100%; final full-parity certification remains reserved for the independent Windows PowerPoint, macOS Keynote, controlled Google Slides, and peer audit.

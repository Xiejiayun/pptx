...
| `slide.addTable()` string / cell-object rows + name/x/y/w/h/colW/rowH | `slide.addTable(rows, options)` | 部分支持：strict rectangular single-paragraph strings 与 exact `{ text: string }` objects、EMU geometry、创建时 scalar/per-axis sizes |

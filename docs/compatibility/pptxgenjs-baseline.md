...
| `slide.addTable()` string / cell-object rows + name/x/y/w/h/colW/rowH/border/fill/margin/valign + cell align | `slide.addTable(rows, options)` | 部分支持：strict rectangular single-paragraph strings、`{ text, options?: { align, border, fill, margin, valign } }` objects、cell align、table/cell border/fill/margin/valign、direct paragraph align 与 cell border/fill/margin/valign、EMU geometry、创建时 scalar/per-axis sizes |
...
| table-cell `align: left/center/right/justify` | `AddTableCellOptions.align` | 已支持单段落创建与 PptxGenJS final-state 对等；table-level align、snapshot/editor 尚未支持 |
...
Table-cell 水平对齐创建通过 `AddTableCellOptions.align` 复用严格的 `TextAlignment`：`left`、`center`、`right`、`justify` 分别映射到该 cell 唯一单段落 direct `a:pPr@algn` token `l`、`ctr`、`r`、`just`，不在 `tcPr` 或 `bodyPr` 写同名属性。omitted 或 runtime-`undefined` 不写 token，与 string cell、`{ text }` 和 empty options 保持原 bytes，也不合成 effective left。该 direct token 在现有 text/margin/border/fill/valign/direction/fit 和 grid/row 编辑、slide duplicate、transaction rollback、write/reopen 中保持不变。PptxGenJS 4.0.1 对四个合法值生成相同 final direct state；未知 runtime 值会被 PptxGenJS 静默丢弃，而原生创建会在 mutation 前抛 `TypeError`。当前不提供 table-level `align`、已有 `TableCell` 水平对齐 snapshot/editor、rich/multi-paragraph cell 对齐或 table default propagation。

...
| `slide.addTable()` string / cell-object rows + name/x/y/w/h/colW/rowH | `slide.addTable(rows, options)` | 部分支持：strict rectangular single-paragraph strings、`{ text, options?: { border, fill, margin, valign } }` objects、direct border/fill/margin/valign、EMU geometry、创建时 scalar/per-axis sizes |
...
| table-cell `valign: top/middle/bottom` | `AddTableCellOptions.valign` / `TableCell.verticalAlignment` / `TableModel.setCellVerticalAlignment()` | 已支持 cell-level 创建与 direct 编辑；table-level 尚未支持 |
...
原生创建字段 `AddTableCellOptions.valign` 和 snapshot 字段 `TableCell.verticalAlignment` 都复用严格的 `top`、`middle`、`bottom`，映射选中物理 cell 的 direct `tcPr@anchor="t/ctr/b"`，绝不读写 cell `bodyPr@anchor`。创建时 omitted/`undefined` 不写 anchor，并与等价 string cell 保持 byte-identical；`marL/marR/marT/marB` 后写 optional `anchor` attribute，再写 L/R/T/B borders 与 fill。live `TableModel` 会立即暴露创建值；getter 要求唯一 direct `tcPr`、唯一无 namespace anchor 和精确 wire token，不合成 effective default。`setCellVerticalAlignment()` 写 canonical token，`undefined` 清除 direct anchor，不支持 `just/dist`。PptxGenJS 4.0.1 对合法 cell-level 值生成相同最终 direct state；完全省略 table/cell `valign` 时不写 anchor，显式 table-level 值会实体化到未覆盖 cells，cell-level 值优先。PptxGenJS runtime 会透传类型外 token，adapter 严格导入为 `undefined`，而原生创建/编辑会直接拒绝。table-level `valign` 创建与 mutation 仍未支持。

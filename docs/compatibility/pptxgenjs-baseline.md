...
| `slide.addTable()` string / cell-object rows + name/x/y/w/h/colW/rowH | `slide.addTable(rows, options)` | 部分支持：strict rectangular single-paragraph strings、`{ text, options?: { border, fill } }` objects、direct border/fill、EMU geometry、创建时 scalar/per-axis sizes |

...
| paragraph 左右 margin、first-line/hanging indent | `paragraphMarginLeft` / `paragraphMarginRight` 与 rich paragraph overrides | 部分支持：direct 左右边距已支持，first-line/hanging 逐项补齐 |
...
Paragraph 左右边距分别通过 `AddTextOptions.paragraphMarginLeft` / `paragraphMarginRight` 提供 plain/rich 创建默认，`RichTextParagraph.marginLeft` / `marginRight` 使用 point 提供本地 number override，`false` 抑制默认或清除对应 direct `pPr@marL` / `marR`。getter 仅读取 `0..51206400` 的 direct integer EMU，不解析继承。direct bullet/numbering/picture bullet 拥有 `marL`，因此 getter 不重复暴露 `marginLeft`，同 paragraph 的 numeric left margin 与 active bullet 也在 mutation 前拒绝；`marR` 与列表缩进独立，可在普通、bullet 或 numbering paragraph 上读取和编辑。左右字段始终映射物理 OOXML sides，不因 RTL 交换。PptxGenJS 4.0.1 没有 paragraph 左右边距公开选项；其普通 paragraph direct `marL="0"`、列表 `marL`/`indent` 和默认缺失 `marR` 的输出均可正常导入。first-line/hanging indent 仍是后续小项。

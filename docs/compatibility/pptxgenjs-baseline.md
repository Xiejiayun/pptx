...
| `slide.addTable()` string / cell-object rows + name/x/y/w/h/colW/rowH/border/fill/margin/valign | `slide.addTable(rows, options)` | 部分支持：strict rectangular single-paragraph strings、`{ text, options?: { border, fill, margin, valign } }` objects、table/cell border/fill/margin/valign、direct border/fill/margin/valign、EMU geometry、创建时 scalar/per-axis sizes |

...
export function normalizeRichText(value: unknown): readonly NormalizedRichTextParagraph[] {
  ...
  return value.map((paragraph, paragraphIndex) => {
    ...
    assertSupportedKeys(
      paragraph,
      ['align', 'bullet', 'level', 'marginLeft', 'marginRight', 'rtl', 'runs', 'spacing', 'tabStops'],
...
export function renderRichTextParagraphs(
  ...
  return paragraphs
    .map(({ align, bullet, level, marginLeft, marginRight, rtl, runs, spacing, tabStops }, index) => {
...
export function renderParagraphProperties(
  ...
  return renderParagraphTabStops(rightMarginalized, prefix, tabStops);
}

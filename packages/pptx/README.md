...
Raster and SVG creation share `name`, `altText`, `rounding`, `transparency`, and strict `ShapeShadow` options. The same live `ImageModel` fields are editable: rounding switches rectangular/ellipse geometry, transparency is a quantized `0..100` percentage, and shadow supports legal outer/inner whole replacement. Equal assignment is an exact no-op; rollback, duplicate isolation, and reopen preserve image relationships and media bytes. PptxGenJS `objectName` maps to native `name`; native shadow preserves legal explicit zero and `rotateWithShape: true` instead of reproducing PptxGenJS falsy/rotate defects.
...
Image hyperlinks support create, read, edit, duplicate, rollback, and reopen. External SVG relationships, SVG DOM-level editing, and broader advanced SVG operations are native-extension boundaries rather than open PptxGenJS 4.0.1 public-surface items. Embedded media creation is documented next.
...
`CHART_TYPES` contains `area`, `bar`, `bar3D`, `bubble`, `bubble3D`, `doughnut`, `line`, `pie`, `radar`, and `scatter`. Categorical charts use `categories` and `values`; scatter uses `xValues` and `values`; bubble and bubble3D additionally require positive `sizes`. Compatible bar/area/line groups can share primary axes or add secondary axes. Every native chart owns an embedded XLSX whose cells, A1 formulas, and display caches are generated from one normalized definition before the synchronous OPC transaction.
...
All ten PptxGenJS 4.0.1 public chart types plus a bar+line primary/secondary combination pass real-output import, semantic edit, formula/cache/XLSX/relationship, and representative option conformance. The final actual-npm aggregate contains 21 slides and 20 native charts; Node, persistent Chrome, declarations, and the installed CLI pass, and PowerPoint 2010 validation reports 0 errors / 0 warnings. The earlier 11-slide / 10-chart visual gallery has zero 180-DPI overflow and passed page-by-page inspection; that historical gallery is not used as the final `bubble3D` / Chart Residual aggregate proof.
...
Office 2016 `cx:*` chart creation/semantic editing, external-workbook editing, chart animations, and broad client certification are native-extension or independent-certification boundaries. The advanced-charts plugin continues to cover modern inspection, trendlines, error bars, and explicit fallback. These boundaries do not leave any PptxGenJS 4.0.1 public-surface item open.
...
Full theme text cascade, broader advanced text/table/media/chart semantic APIs, and broader client certification remain possible native extensions, but are not open PptxGenJS 4.0.1 public-surface items. Ordinary shape/text percentage coordinates, shared editable `name`, rotation, and horizontal/vertical flips are closed. Advanced text now includes text-shape direct fill, simple line, begin/end arrows, simple shadow, outer and per-run hyperlinks, preset geometry, rounded-rectangle radius, direct `isTextBox` state, and rich-text `breakLine` paragraph splitting.
...
outlined.line = undefined; // clears only direct width/fill/dash/cap
```

`AddTextOptions.line` reuses the strict `ShapeLine` union. It accepts only `{ kind: 'none' }` or `{ kind: 'line', color, transparency?, width?, dash?, cap? }`: color is strict six-digit sRGB or a supported scheme color, transparency is finite `0..100` rounded to `0.001%`, width is finite `0..1584` points rounded to one EMU, dash is one of `solid/dash/dashDot/lgDash/lgDashDot/lgDashDotDot/sysDash/sysDot`, and cap is `flat/round/square`. Omitted width/dash materialize as 1pt/solid; zero width and explicit zero transparency retain direct intent. Omitted, runtime-`undefined`, and explicit none creation preserve the existing canonical `<a:ln><a:noFill/></a:ln>` text-box default.
...
This is the historical checkpoint at completion of rich-text `breakLine`; at that point, advanced text/table, `tableToSlides`, output/runtime helpers, and peer/client audit work still remained before full PptxGenJS parity could be claimed.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. Six-value `write({ outputType })` return semantics are completed in the next section.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. Node readable output is completed in the next section.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. Compression policy is completed in the next section.
...
At this historical checkpoint, overall PptxGenJS parity was approximately 97%. The `SchemeColor` runtime helper is completed in the next section.
...
`ShapeLine` supports `{ kind: 'none' }` or a strict solid line with six-digit sRGB/theme color, optional finite 0–100 transparency, optional 0–1584 point width, optional `solid | dash | dashDot | lgDash | lgDashDot | lgDashDotDot | sysDash | sysDot` dash, and optional `flat | round | square` cap. Omitted width/dash materialize as 1pt/solid, while zero width is preserved. `ShapeModel.line` returns a detached direct-state snapshot; same-value assignment is an exact no-op, none writes direct line no-fill, and `undefined` clears only owned width/fill/dash/cap while preserving the line container, arrowheads, joins, extensions, and unrelated attributes. Unique gradient/picture/pattern/group line fill or custom dash can be explicitly replaced or cleared, but their creation is outside this simple-line API. PptxGenJS 4.0.1 maps its `dashType` and `cap` to native direct state; its omitted/explicit-none line, empty/missing-color fallback to `333333`, zero-width fallback to 1pt, omitted direct alpha for zero transparency, and ignored deprecated `alpha`/`lineDash` differ from native strict direct-intent semantics.
...
PptxGenJS 4.0.1 has creation-time table rows, `rowH` / `colW`, and auto-page helpers but no existing-deck row/column editor. Tables produced through its public API can be imported and edited with these native lossless methods. Auto-page, repeated headers, and automatic content measurement/layout recomputation are supported below; this structural-edit API still does not insert logical content, while HTML import is covered by the separate `tableToSlides()` API.
...
Native does not copy PptxGenJS 4.0.1 caller mutation, coercion/clamping, or existing-following-slide reuse. This is the historical 99.7% measurement checkpoint; `tableToSlides` is completed in the following section. Current public capability coverage is 100%, while final full-parity certification still waits for the peer/client audit.
...
## Create editable slides from an HTML table

In a browser, `await document.tableToSlides(elementId, options)` snapshots a `<table>` by ID and returns the frozen list of ordinary editable `SlideModel` pages it generated.
...
const table = pages[0]!.shapes.find((shape) => shape instanceof TableModel) as TableModel;
table.setCellText(1, 0, 'Edited after import');
```

Rows are ordered as `thead`, every `tbody`, then `tfoot`. Cell computed CSS maps sRGB foreground/background, font family/size/weight, horizontal and vertical alignment, padding, and four borders. Explicit `columnWidths` wins; otherwise visible pixel ratios plus header `data-pptx-width` / `data-pptx-min-width` constraints produce positive EMU columns with an exact total. Those two HTML attributes remain inch-valued for PptxGenJS compatibility; native geometry is EMU.

`autoPage` defaults to true and reuses measured rows, repeated headers, named-layout margins, layout/section/slide-number ownership, and page-local links. Explicit false creates one ordinary automatic-row table and rejects auto-page-only controls. Per HTML page, additions commit in image → shape → table → text order. Images resolve once, retain page-local relationships, and reuse only exact content-type-and-byte media payloads. DOM, CSS, columns, and images preflight before package mutation; all generated slides and additions commit atomically.

Boundaries: this is a browser-DOM API; it snapshots `innerText`, not word-level HTML, and maps only cell computed styles. Color alpha is not retained for table fill/borders, visible non-solid borders become dash, and a fully hidden flexible table is rejected. Native deliberately avoids PptxGenJS 4.0.1 selector interpolation, caller mutation, ignored `autoPage: false`, truthy coercion, fixed-width-as-minimum behavior, and silent invalid numbers.

The HTML `tableToSlides` workstream is 10/10 complete. Its focused gate covers 4 files / 463 tests in 57.92s; the full gate covers 91 passed / 1 skipped test files and 1764 passed / 1 skipped tests in 102.00s; the independent performance gate is 1/1 with a 1.145s core test and 3.10s Vitest duration. TypeScript, root build, and package build complete in 2.46s, 2.48s, and 18.14s.

Two 63-file dist manifests are identical with SHA-256 `280e78ad56ad3cb5891b80a729d2a899bfd42f3e77648517c1982df71a63bc3a`; two 66-entry, 712,812-byte actual tarballs are byte-identical with SHA-256 `2fbd19e234d9e92591c0825acd6082bfda394ff871aa58d715c20d71f2e1d623`. Installed Node, NodeNext declarations, the browser conditional export, CLI, and Inspector pass with `tableToSlides: true` / `tableToSlidesInspect: true`. Google Chrome 150.0.7871.188 reports every create/style/width/header/layout/addition/relationship/edit/reopen state true, zero validation errors, and 0/0/0 console/page/network errors.
...
The declaration-atom matrix closes all 1,774/1,774 entries (100%): 764 supported, 542 deliberate differences, 94 deprecated aliases, and 374 defect exclusions. Unsupported, unverified, stale, and diagnostic entries are all zero. The final Chart Residual family closes 23 atoms; native, the actual npm tarball, and persistent Chrome share the same 23-atom / 33-state probe, and all six presentation formats, 10 charts / 10 workbooks, exact OOXML checks, and PowerPoint 2010 validation pass.

Two audit generations are byte-identical and report `complete=true`. Windows PowerPoint, macOS Keynote, and controlled Google Slides import remain independent client-certification work rather than missing PptxGenJS 4.0.1 public-surface implementation.
...
PptxGenJS 4.0.1 public-surface parity is complete: 1,774/1,774 declaration
atoms are evidence-closed, with zero unsupported, unverified, stale, or
diagnostic entries. Native supports creating complete presentations from zero
and semantically editing existing presentations; deliberate differences use a
strict or corrected native representation instead of reproducing permissive,
inert, or defective PptxGenJS behavior.

...
Google Chrome 150.0.7871.188 reports create/read/frozen snapshot/unmerge/edit/remerge/reopen true with zero validation, console, page, or network errors. The browser evidence deck has 18 parts / 15 relationships, 1 slide / 1 table, a 2×3 physical-cell matrix, and one 2×2 merge region. All four anchor/continuation token forms are present, the slide relationship part contains only its valid layout owner, and PowerPoint 2010 validation is 0 errors / 0 warnings. Recognition, creation, snapshot/editor, SDK/adapter, documentation, and package-proof commits are `688f9f6`, `3d93f07`, `db01937`, `b2f6846`, `5832399`, `7073eae`, and `f174519`; evidence is retained at `/tmp/pptx-table-cell-merges-artifacts.B7ZhGQ`. Physical row/column CRUD and explicit-row-height auto-page/repeated headers are completed in the following sections. Overall PptxGenJS parity is approximately 99.3%; automatic content measurement/layout recomputation, `tableToSlides`, and the final peer/client audit remain.
...
PptxGenJS 4.0.1 has creation-time table rows, `rowH` / `colW`, and auto-page helpers but no existing-deck row/column editor. Tables produced through its public API can be imported and edited with these native lossless methods. Explicit-row-height auto-page/repeated headers are supported below; logical content insertion, automatic content measurement/layout recomputation, and `tableToSlides` remain pending.
...
console.log(document.slides.indexOf(following)); // 3; generated slide is between source and following
```

This specialty performs deterministic structural pagination. `autoPage: true` requires one positive EMU `rowHeights` value for every physical row. The source page starts at the table's own `y`. Continuation pages start at `autoPageSlideStartY`, the current runtime named-layout top margin, or the canonical 0.5-inch margin, in that order, and stop before the bottom margin. Explicit `slideMargin` is either one non-negative EMU value or an exact `[top, right, bottom, left]` tuple and takes precedence over layout margins.

When `autoPageRepeatHeader` is true, `autoPageHeaderRows` defaults to one and may select multiple header rows. Rowspan blocks remain atomic, and a merge may not cross the header/body boundary. Every page contains an ordinary editable table with rich text, cell styles, URL/internal-slide links, column widths, merge tokens, and page-local relationship ownership preserved. Internal slide targets are resolved before inserting any continuation slide.
...
Automatic/omitted row heights, `autoPageCharWeight`, `autoPageLineWeight`, text-row fragmentation, placeholder auto-page, content measurement/layout recomputation, and `tableToSlides` are not supported yet and are rejected rather than guessed. Native also does not copy PptxGenJS 4.0.1 caller mutation, coercion/clamping, or existing-following-slide reuse. Overall PptxGenJS parity is approximately 99.3%; full parity is not yet claimed.

...
`AddTableCellOptions.hyperlink` accepts a URL or a one-based current-presentation slide target. Omitted tooltip and explicit `''` remain distinct, and readonly `TableCell.hyperlink` immediately returns a detached frozen direct-state snapshot. The link belongs to the plain cell's only direct run; `setCellText()` and supported cell-property edits preserve it. Internal relationships retain target-slide identity, so moving the target updates the reported ordinal. Existing duplicate, delete, rollback, six-format, and write/reopen relationship lifecycles apply.

Every linked cell owns an independent relationship even when multiple cells use the same URL. There is no table-level hyperlink default and no table-cell hyperlink editing or clearing API yet. The strict reader accepts only one direct text body, paragraph, run, and run-properties path; rich, multi-run, and multi-paragraph cell links remain unsupported. Legal PptxGenJS 4.0.1 URL/slide output imports, including its extra `invalidUrl=""`, `action=""`, and `history="1"` run-click attributes. PptxGenJS materializes omitted tooltip as empty and writes `_rId` back into caller hyperlink objects; native preserves omitted versus empty state and never mutates input.
...
Overall PptxGenJS parity remains approximately 97%. The next item is table-cell hyperlink editing/clearing, followed by rich/multi-paragraph cell text and style, merge/colspan/rowspan, row/column CRUD, auto-page/repeated headers, `tableToSlides`, and the final peer/client audit.
...
Creation is native and does not require PptxGenJS. New documents include the complete default presentation chain and support preset or custom EMU slide sizes, direct presentation RTL, editable canvas size, plain/rich text, strict shapes, media, charts, and tables. `slide.addTable()` accepts strict rectangular single-paragraph strings or `{ text, options? }` cells, table/cell align, border, fill, margin, text direction and vertical alignment defaults, cell fit, exact geometry, column widths, and row heights. Creation defaults materialize only into uncovered physical cells and are not retained as metadata. Existing tables expose live cell snapshots/editors, strict grid/row editing, and uniform direct table-level consensus/bulk editing for vertical alignment, text direction, horizontal alignment, margins, borders, and fill. Explicit `horz`, direct no-fill, and explicit-zero alpha remain distinct from `undefined` clear; mixed detail stays in `rows[].cells[]`, exact no-ops preserve bytes/journal, and unsafe edits reject without partial mutation. Table-level fit, diagonal/advanced borders and fills, rich/multi-paragraph cell text, hyperlinks, merges, row/column CRUD, creation styles, auto-page/repeated headers, content measurement, and layout recomputation remain pending. The dedicated sections above and below define exact creation precedence, numeric ranges, OOXML mappings, PptxGenJS boundaries, and the wider text/shape/media APIs.

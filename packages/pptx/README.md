...
`AddTableCellOptions.textDirection` accepts the strict table-cell values `horz`, `vert`, `vert270`, and `wordArtVert`. Creation owns direct physical-cell `tcPr@vert`: omitted, runtime-`undefined`, and explicit `horz` write no attribute, while the three non-horizontal values write their exact token and appear immediately in `TableCell.textDirection`. The value is normalized descriptor-safely, detached immediately, and survives supported cell edits, duplication, rollback, write, and reopen. `setCellTextDirection()` deliberately retains direct-editor semantics, so assigning `horz` writes `vert="horz"` and assigning `undefined` clears the attribute. Supported native creation matches PptxGenJS 4.0.1 final state; native rejects its invalid runtime-token passthrough. A table-level direction default remains pending.

...
Creation writes the same relationship to the non-visual shape click and every non-empty text run; empty paragraphs and runs do not synthesize links. Linked runs default to single underline, while an explicit `RichTextRunStyle.underline` wins, including `false`. `ShapeModel.hyperlink` continues to read and edit only the whole-shape click: whole replacement, clear, exact no-op, relationship clone-on-write, and garbage collection do not overwrite pre-existing run links.

URL/internal-slide targets and tooltip state survive duplicate, move, delete, transaction rollback, all six formats, and write/reopen. Duplicate self-links retarget to the duplicate, while deleting a target cleans incoming shape/run clicks. PptxGenJS 4.0.1 materializes omitted tooltip as `tooltip=""`; rich outer hyperlink emits broken `rIdundefined` references, while rich per-run hyperlinks write only run links and allocate one relationship per run. Invalid runtime values may be loosely converted, left dangling, or reported only to the console. Native emits one legal shared relationship for outer hyperlinks and rejects invalid input with zero mutation.

Final full Vitest is 1290 passed / 1 skipped and performance is 1/1 at 584ms. Model, SDK, root, and adapter suites are 196/196, 189/189, 12/12, and 80/80; TypeScript, both tsup builds, and declaration build pass. The actual 57-file tarball plus installed Node/declarations/browser/CLI and real Chrome report `textShapeHyperlinks: true`; Chrome has zero validation/console/page/network errors. The internal-only installed-CLI PowerPoint 2010 profile is 0 errors / 0 warnings, while external links produce only the expected `OPC_EXTERNAL_RELATIONSHIP` portability warning. Per-run rich-text hyperlink create/read/edit remains unsupported and is next.

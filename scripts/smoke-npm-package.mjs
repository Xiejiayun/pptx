...
const createdTable = tableSlide.addTable([['Region', 'Revenue'], ['East', '']], { name: 'Created smoke table', columnWidths: [inches(1), inches(3)] });
const createdTableDefaults = createdTable instanceof TableModel && createdTable.transform.x === inches(0.5) && createdTable.transform.y === inches(0.5) && createdTable.transform.height === inches(1) && createdTable.rows[0].cells[0].margins?.top === 3.6 && createdTable.rows[0].cells[0].margins?.left === 7.2 && createdTable.rows[0].cells[0].borders?.top?.kind === 'none';
...
const tableCreation = createdTableDefaults && reopenedCreatedTable instanceof TableModel && reopenedCreatedTable.rows[1].cells[0].text === 'Edited East' && reopenedCreatedTable.rows[1].cells[1].text === '' && reopenedCreatedTable.rows[1].cells[1].verticalAlignment === 'bottom';
...
const tableSlide = created.slides[0];
const createdTable = tableSlide.addTable([['Region', 'Revenue'], ['West', '']], { name: 'Created browser table', columnWidths: inches(1.25) });
...
if (!(createdTable instanceof TableModel) || createdTable.transform.x !== inches(0.5) || createdTable.transform.y !== inches(0.5) || createdTable.transform.width !== inches(2.5) || createdTable.transform.height !== inches(1) || createdTableGrid.length !== 2 || createdTableGrid.some((width) => width !== inches(1.25)) || createdTable.rows[0].cells[0].margins?.top !== 3.6 || createdTable.rows[0].cells[0].borders?.left?.kind !== 'none') throw new Error('Browser table column-width creation failed');
createdTable.setCellText(1, 0, 'Edited West');
createdTable.setCellFill(0, 0, { kind: 'solid', color: { kind: 'scheme', value: 'accent1' } });
const reopenedCreated = await PptxDocument.open(await created.write());
const reopenedCreatedTable = reopenedCreated.slides[0].shapes.find((shape) => shape.name === 'Created browser table');
if (!(reopenedCreatedTable instanceof TableModel) || reopenedCreatedTable.transform.width !== inches(2.5) || reopenedCreatedTable.rows[1].cells[0].text !== 'Edited West' || reopenedCreatedTable.rows[1].cells[1].text !== '' || reopenedCreatedTable.rows[0].cells[0].fill?.kind !== 'solid') throw new Error('Browser table creation round trip failed');
const tablePart = created.opcPackage.requirePart(tableSlide.partUri);
...
const tableOptions: AddTableOptions = { name: 'Typed table', x: inches(1), columnWidths: [inches(1), inches(3)] };

...
const createdText = created.addSlide().addText('Smoke\\n\\nParagraph', { align: 'center', bullet: true, level: 2, margin: 10, spacing: { before: 4, after: 6, line: { kind: 'exact', points: 20 } }, tabStops: [{ position: 1.25 }, { position: 2.5, alignment: 'right' }] });
...
const browserText = created.addSlide().addText('Browser\\nText', { align: 'center', bullet: true, level: 2, margin: [0, 0, 0, 0], spacing: { line: { kind: 'multiple', factor: 1.25 } }, tabStops: [{ position: 1.25 }] });
if (browserText.richText[0].tabStops[0].position !== 1.25 || browserText.textMargins.top !== 0 || browserText.textMargins.right !== 0 || browserText.textMargins.bottom !== 0 || browserText.textMargins.left !== 0) throw new Error('Browser create-text API failed');
...
const createdText = createdDocument.addSlide().addText('Typed\\ntext', { align: alignment, bullet, level: 2, margin, spacing, tabStops });
...
void [documentPromise, createdDocument, customDocument, createdText, marginSnapshot, richText, gradientConstructor, adapter, transition, animationConstructor, chartConstructor, smartArtConstructor];
`,
  );
  run(
    process.execPath,
